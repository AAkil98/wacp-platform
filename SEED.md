...
**Branches.** `main` and `dev` both at `338e4f2` (ff'd 2026-04-20). Today's ff carried 11 commits forward: the 2026-04-19 **closeout-plan P1** batch (6 phases closing `git-strategy.md` §13 tooling items + `tech-debt-2026-04-18.md` §3.1 Bucket A + Bucket C guardrail) plus two post-batch SEED refreshes plus **closeout-plan P2** (branch protection configured on `main` + `dev` via `gh api`, commit `338e4f2`). Full P1/P2 log at `impl/closeout-plan.md` §7. The prior (2026-04-18) ff from `d0be941` carried 48 commits spanning §13.7.6b / §13.7.9 / §13.7.7 D1–D5 / §2.1–§2.6 / §2.7 full-fix / §2.7.1b / protoc rate-limit / §13.7.8 I1–I5 / §11.4 P0 + `tech-debt-2026-04-18.md` + `impl/git-strategy.md` — every AUDIT §13.7 package except 13.7.10 closed.

**CI state.** Four workflows (`ci-lint`, `ci-wacp`, `ci-console`, `coverage`) run on each push to `main` + `dev`. Docs-only commits path-filter down to `ci-lint` + `coverage`. Last applicable runs on `dev` at tip `338e4f2` were green; first post-ff run on `main` at `338e4f2` is in flight at refresh time — use `gh run list --branch main --limit 6` to verify. `ci-console` includes the Playwright `e2e` stage + 50-test integration suite; `ci-lint` `file-size` job (added in P1) passes (6 warnings / 0 fails). Branch protection now active on `main` + `dev` (linear-history + no force-push + no deletion; `enforce_admins=false` for solo-maintainer emergency bypass). Remaining CI tasks are future-facing (Playwright `--coverage` merge into lcov, Codecov ratchet).
...
- `impl/closeout-plan.md` — five-phase plan closing the nine open items. **P1 landed 2026-04-19** (range `6197263..c07af7f`); **P2 landed 2026-04-20** (`338e4f2`); **P3 ff'd 2026-04-20** (dev→main aligned at `338e4f2`). P4 (Bucket B refactor) + P5 (§12.5 bisect) remain. §7 execution log has per-phase SHAs.

**When resuming:**

**Branch state to start:** `git checkout dev`. Check tips with `git rev-parse dev main` + `git log --oneline main..dev | wc -l` (expect 0 or 1 post-2026-04-20 ff — any SEED-refresh commit sits on dev until the next ff). Check CI with `gh run list --branch main --limit 6` (ff'd 2026-04-20) or `--branch dev --limit 4`. Topic branch `ci/pre-launch-closeout` was ff'd to dev + deleted locally 2026-04-19; the remote copy at `aakil98/ci/pre-launch-closeout` is still live (explicit delete pending user go-ahead).
...
4. ~~**Closeout-plan P2 — branch protection**~~ — **configured on `main` + `dev` 2026-04-20** via `gh api`. Linear history + no force-push + no deletion; `enforce_admins=false`. See `impl/git-strategy.md` §11.5 for the revised posture.
5. ~~**Closeout-plan P3 — dev → main ff**~~ — **done 2026-04-20**. Aligned at `338e4f2`; CI on `main` in flight at refresh time.
6. **Closeout-plan P4 — Bucket B refactor** (next). ~8–12 h on `refactor/file-splits` branched from `main` (post-P3). Split 9 oversized files per `tech-debt-2026-04-18.md` §3.2. Single blame event, behavior-preserving.
7. **Closeout-plan P5 — §12.5 `ProfilesPage` Create-New unmount** — 30–60 min bisect, per `HEALTH-LOG.md` §12.5. Orthogonal to P4.
8. **Playwright `--coverage` merge** — 1–2 h: wire Playwright's `--coverage` → `wacp-console/frontend/coverage/lcov.info`. Deferred during D3; pick up when baseline numbers settle.

**Independent / scheduled:**
9. **§13.7.10 (Codecov monthly ratchet)** — still deferred until 2–3 `main` merges land the §13.7.6b + §13.7.7 + §13.7.8 + §2.7 + closeout-plan batch so the baseline settles. The 2026-04-20 ff counts as one; two more land as Bucket B refactor + §12.5 bisect close.
...
**Merge strategy:** dev and main aligned at `338e4f2` post-2026-04-20 ff. Next natural cut point is the Bucket B refactor (closeout-plan P4) on `refactor/file-splits` post-P3 — single reviewable PR, behavior-preserving. Tag `wacp-runtime-v0.1.0` / `wacp-console-v0.1.0` once Rust branch-coverage floor clears 85% and first mutation run hits ≥85% per module (both post-v0.1 gates, not pre-merge). See `impl/git-strategy.md` §8 for full tagging ceremony + v0.1.0 gates.
