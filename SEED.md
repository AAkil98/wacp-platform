...
9. ~~**Bucket-B follow-up production splits**~~ — **done 2026-04-20**: `routes/highway.rs` 1302 → 536 via `highway_pending.rs` extract (`b9dfaf1`); `config.rs` 1038 → 860 via `config_env.rs` extract (`27789ff`). `.file-size-allowlist` Rust production section now empty. Bucket-B plan archived.
