...
**Branch:** `dev`, **47 commits ahead of `main` @ `743c9bd`**. The dev→main batched merge ran 2026-04-17 morning (fast-forward, 21 commits); since then dev has accumulated §13.7.6b WA1/WA2/WA3 + strategy-doc + seed refresh + WA3.5/WA3.6 + WA5 + T7.3 (with WorkspaceState fix) + un-ignore sweep + closure docs + §13.7.9 mutation-testing pipeline + §13.7.7 D1 (Playwright tooling) + inter-deliverable drift-filing + §13.7.7 D2 (five E2E spec files + three backend-drift fixes) + **§2.1–§2.6 CI-pipeline cleanup** + **§2.7 full-fix** (Phases A+B+C+D+E, all 9 drift items + D.1/D.2 debt, ff'd from `ci/cleanup-2.7`) + **§2.7.1b** (14 highway-ui strict-mode errors unmasked by §2.7.4) + **protoc rate-limit hardening** (`repo-token` on 12 `arduino/setup-protoc@v3` usages) + **§13.7.7 D3** (Playwright `e2e` job in `ci-console.yml`) + D4 (e2e/README.md) + D5 + **§13.7.8 I1–I5** (Rust integration + chaos — P0 shared `InjectableCoordinator` mock + 5 new suites, 36 new tests, 50/50 integration green) + **§11.4 P0 pass** (four new `_to_proto` helpers + 7 broken casts fixed in `wacp-runtime/src/init.rs`; one recovery-matrix test coincidentally-green, renamed + flipped) + **planning docs** (`impl/tech-debt-2026-04-18.md` baseline survey + 3-bucket triage + file-size-allowlist proposal; `impl/git-strategy.md` codifying branching/commit/merge/CI/release conventions in use). **§13.7.6b + §13.7.9 + §2.1–§2.6 CI cleanup + §2.7 in full + §13.7.7 in full + §13.7.8 in full + §11.4 P0 all closed.** Working tree clean.

**CI state on `dev`:** all four triggered workflows (`ci-lint`, `ci-wacp`, `ci-console`, `coverage`) are **green**. The `ci-console` workflow includes the new `e2e` (Playwright) job + the Integration job runs the 50-test integration suite (10 files) in ~2 min. Remaining CI tasks are future-facing (Playwright `--coverage` merge into lcov, Codecov ratchet); nothing red on `dev`.
...
**CI state on `dev`:** all four triggered workflows green. `ci-console` runs Rust + Integration (now 50 tests) + Frontend + E2E (Playwright). Dev is **47 commits ahead of main** @ `743c9bd`.

**Key docs:**
- `impl/ci-health-2026-04-17.md` — historical record of the §2.1–§2.7 cleanup.
- `impl/ci-cleanup-2.7-plan.md` — five-phase CI-drift plan; documents four plan deviations that needed adapting.
...
- `impl/tech-debt-2026-04-18.md` — baseline survey (file sizes, binary sizes, dep graph, disk footprint) + 3-bucket triage (A pre-launch deletion / B post-v0.1 refactor / C CI prevention). User-facing footprint is fine; contributor-facing is the problem. Open questions: delete `wacp/highway-ui/`?, lint thresholds, refactor PR granularity.
- `impl/git-strategy.md` — branching (main / dev / `{scope}/{slug}` topic branches), commit conventions extending CLAUDE.md with §X.Y scopes, fast-forward merges, draft-PR-for-CI pattern, release tagging per binary, failure-handling recipes. Flags one pre-launch gap: **GitHub branch protection on `main` + `dev` is not configured yet.**

**When resuming:**

**Branch state to start:** `git checkout dev`. Working tree clean. All four CI workflows green on dev; 47 commits ahead of main.
...
2. **Tech-debt doc — open questions from `impl/tech-debt-2026-04-18.md` §7** — three user decisions pending: (a) delete `wacp/highway-ui/`? (269 MB node_modules + 8.6k source lines + its own CI stage, not shipped to users); (b) file-size lint thresholds (proposed: warn >1000 / fail >1500 Rust, warn >500 / fail >1000 TS); (c) refactor branch granularity — one PR or many?
3. **Pre-launch items from tech-debt doc §3.1 Bucket A + `impl/git-strategy.md` §13** — Vite `sourcemap: true` → `false` (5 min, removes 1.7 MB from embedded binary + info-leak); GitHub branch protection on `main`/`dev` (10 min repo settings, closes §11.5 risk class); optional `.gitmessage` template + pre-push hook.
4. **§12.5 `ProfilesPage` Create-New unmount** — 30–60 min bisect, per `wacp-console/performance-optimization.md` §12.5.
5. **Playwright `--coverage` merge** — 1–2 h: wire Playwright's `--coverage` → `wacp-console/frontend/coverage/lcov.info`. Deferred during D3; pick up when baseline numbers settle.
6. **Merge `dev` → `main`** — dev is now **47 commits ahead** with every CI check green. **Past the 10–40-commit natural-batch-close heuristic** in `impl/git-strategy.md` §5.3; the batch is ripe and ff should happen before more work accumulates.

**Independent / scheduled:**
7. **§13.7.10 (Codecov monthly ratchet)** — still deferred until 2–3 `main` merges land the §13.7.6b + §13.7.7 + §13.7.8 + §2.7 batch so the baseline settles.
8. **§13.7.9 first-run triage** — next Monday 04:00 UTC cron (surviving mutants → killer tests; equivalent → `// mutants:skip`).
...
10. **Post-v0.1 refactor (`impl/tech-debt-2026-04-18.md` §3.2 Bucket B)** — ~8–12 h on a dedicated `refactor/file-splits` branch. Split the 9 oversized production files (`init.rs` 2139, `session_monitor.rs` 2120, `session_launcher.rs` 1877, `highway.rs` 1832, `config.rs` 1748, `recovery.rs` 1485, `rest_gateway.rs` 1202, `sessions.rs` 1181, `execution.rs` 1137). Single blame event, behavior-preserving.

**Merge strategy:** dev @ 47 commits ahead of main, all CI green. Ready to ff whenever convenient per `impl/git-strategy.md` §5.3. Tag `wacp-runtime-v0.1.0` / `wacp-console-v0.1.0` once Rust branch-coverage floor clears 85% and first mutation run hits ≥85% per module (both post-v0.1 gates, not pre-merge). See `impl/git-strategy.md` §8 for full tagging ceremony + v0.1.0 gates.
