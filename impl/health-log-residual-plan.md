...
|---|---|---|---|
| P1.a | `9f85f67` | 2026-04-22 | Relocate `mock_coordinator.rs` → `console-test-support`; decouple `spawn()` from `RuntimeHarness` (now takes `impl Into<String>` upstream-addr, breaks the cyclic dep console-test-support ↔ console-integration); add `wacp-transport` to console-test-support deps; re-export from integration/src/lib.rs. 2 test call sites updated. Kept `console-test-support` in integration's `[dependencies]` (not dev-deps) since the re-export lives in `src/lib.rs`; unpublished crate, no leak. `mock_coordinator_smoke` 2/2 + `launch_failure_matrix` 10/10 green. |
| P1.b | `0d24538` | 2026-04-22 | **Plan deviation:** bench uses `ProgrammableCoordinator` (standalone canned-response mock, already in `console-test-support`) rather than the newly-relocated `InjectableCoordinator` (which forwards to a real `wacp-runtime` child process — wrong shape for a bench hot loop). Criterion sweep at N ∈ {1, 3, 10, 30}; `iter_custom` excludes per-sample setup from timing; atomic u64 counter outside the closure keeps session_ids unique across criterion's multi-invocation sampling (caught via UNIQUE-constraint panic on first run, fixed before landing). Numbers: N=1 2.94 ms / N=3 5.40 ms / N=10 13.6 ms / N=30 34.2 ms — linear scaling ~1 ms per extra assignment above a ~2 ms gRPC/DB floor. Recorded in `docs/perf-baseline-2026-04-20.md` §session_launcher_launch; placeholder bullet struck. |
| P2 | `d99e896` | 2026-04-22 | Initial narrow grep (scaffolding session's `match.*proto::(GateType\|...)` pattern) returned zero hits; broader grep `proto::[A-Z]\w*::\w* =>` surfaced 5 sites. Two are truly exhaustive (no `_ =>`): `event_enricher::gate_type_string` (8-variant `proto::GateType`) + `session_monitor::workspace_state_string` (10-variant `proto::WorkspaceState`). Three have intentional wildcards (narrowing / default-map patterns): `session_monitor.rs:518-521`, `recovery.rs:173-176`, `routes/highway.rs:66-71`. Also found `recovery::is_terminal` using `matches!` with OR — not a match expression; out of scope. Per plan, only the two truly-exhaustive sites got the comment. Clean classification documented in commit body. |
| P3 | `740e2a0` | 2026-04-22 | Four AUDIT edits: §13.7.8 I4 row strike with inline rationale, §12.5 prose "four → three sub-scenarios remain deferred", §13.7.8 numbered-list item 3 accepted+struck, §13.4 drift bullet closed. §13.9 table row 13.9.9 covers all four plan phases under one closure entry (consistent with §13.9.3/§13.9.7/§13.9.8 bundling precedent). Footer appended. Skipped the plan's optional `wcon-highway` §4.3 clarification — spec-repo edit, and §4.3 already carries the "authoritative signal to client" wording. |
| P4 | (this commit) | 2026-04-22 | HEALTH-LOG strikes: §8 "Placeholder: session_launcher_bench..." paragraph, §11.2 "Recommendation" paragraph, §13.4 option-1 "Build it" entry + Resolution paragraph added. Plan archived to `impl/archive/health-log-residual-plan.md` via `git mv`. Cross-reference check: SEED still references `impl/health-log-residual-plan.md` — no; SEED was refreshed 22nd-pass before this plan and only flagged the two scaffolded plans via commit `1b3cb69`, not a path-ref. AUDIT §13.9.9 already points at `impl/archive/...`. |
