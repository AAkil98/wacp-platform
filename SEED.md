...
**Branches.** `main` = `dev` = `f2f486a` (from the 2026-04-22 02:22 UTC ff). Use `git log --oneline main..dev | wc -l` for the live divergence count (0 at refresh time).
...
**CI state.** Four push-triggered workflows (`ci-lint`, `ci-wacp`, `ci-console`, `coverage`) plus one cron-only (`ci-mutation`). Path filters narrow docs-only commits to `ci-lint` + `coverage`, and wacp-only commits to `ci-wacp` + `ci-lint`. Both branches at `f2f486a`: dev CI all 4 workflows green on tip; main CI triggered at ff-push time, running/queued at SEED-write. Every prior post-audit batch (listed above) ran green through CI on dev before ffing. New CI jobs now in place: `mutation-scripts` parser-regression test in `ci-lint` (from test-infra-followups Phase C), `frontend-e2e` Playwright V8 coverage job in `coverage` (Phase A), **`--all-targets` on both Rust clippy jobs** (from test-cleanup-followups Phase A.2 — any `#[cfg(test)]`-only drift now fails per-PR instead of accumulating behind the default-target gate). `ci-lint` `file-size` job passes (6 warnings / 0 fails). Branch protection active on `main` + `dev` (linear-history + no force-push + no deletion; `enforce_admins=false`).
...
**Every AUDIT §13.7 work package except §13.7.10 is closed, and every "Primary track" from the 20th pass is now closed.** All §13.7.1–§13.7.9 packages on `main`; AUDIT §13.9.1/§13.9.2/§13.9.3/§13.9.4/§13.9.5/§13.9.6/§13.9.7/§13.9.8 closed. All four mutation targets at 100 % locally. Test totals post-session: `wacp-transport` 209, `wacp-tools` 136, `console-core` 190, `console-db` 96, `console-integration` 53 passing / 0 ignored (+1 from `terminal_workspace_closed_marked_completed`), `pnpm test:e2e` 7 pass / 9 skip / 0 fail (unchanged), frontend `pnpm typecheck` + `pnpm lint` 0 errors, `pnpm test` 124 (3 shards), `wacp-coordinator` 387, `wacp-workspace` 65, `wacp-runtime` 109, `wacp-types` 45, `wacp-local` 86, `wacp-cli` 132, `sdk-python` 104.

**CI state on `dev` and `main`:** both at `f2f486a`, 0 divergence (ff'd 2026-04-22 02:22 UTC). Dev CI all 4 workflows green on tip; main CI triggered at ff-push, running/queued at SEED-write. All 6 post-21st-pass batches (console-db-schema-alignment, test-infra-followups, public-facing docs, test-cleanup-followups) CI-green on their respective SHAs before ff. New jobs active: `mutation-scripts` (ci-lint parser regression), `frontend-e2e` (Playwright V8 coverage in coverage workflow), **`--all-targets` Rust clippy** (both ci-wacp + ci-console — from test-cleanup-followups A.2). Mutation pipeline (`ci-mutation`) schedule/`workflow_dispatch`-only — does not block PRs.
...
**Branch state to start:** `git checkout dev`. Confirm tips with `git rev-parse dev main` and divergence with `git log --oneline main..dev | wc -l` (0 at SEED-write time — just ff'd). Check CI on tip with `gh run list --branch dev --limit 6` or `--branch main --limit 6`. No live topic branches at refresh time; remote copies of all prior-closed branches have been deleted through 2026-04-22 (`testing/cleanup-followups` was deleted at ff time).

**Primary tracks — every closeable item from the prior-session Resumption Point is closed.** The 20th-pass list (session_monitor, Playwright coverage merge, pick_port TOCTOU, mutation pipeline hardening, Closed-terminal coverage, single-survivor mutants) is 100 % struck. The 21st-pass open follow-ups (HEALTH-LOG §16 clippy-test-drift, Closed-terminal coverage) both closed 2026-04-22 via `impl/archive/test-cleanup-followups-plan.md`.
...
2. **Four §13.7.8 deferred sub-scenarios** — each tied to absent infrastructure (mock highway, FaultyDb::drop_reads, REST replay endpoint, schema-evolution fixture). In-file notes in the relevant tests.

**Independent / scheduled:**
3. **§13.7.10 (Codecov monthly ratchet)** — deferred until 2–3 more `main` merges settle the baseline. The 2026-04-22 ff carried a large batch forward; one or two more ffs and the baseline should be ready to ratchet.
4. **§13.7.9 Monday cron** — next scheduled run is 2026-04-27 04:00 UTC. Under the new `^27` pin + parser regression test.

**Merge strategy:** dev = main = `f2f486a` (ff'd 2026-04-22). Next batch starts from this baseline. Tag `wacp-runtime-v0.1.0` / `wacp-console-v0.1.0` once Rust branch-coverage floor clears 85 % and mutation targets stay ≥85 % (both post-v0.1 gates, not pre-merge). See `impl/git-strategy.md` §8 for full tagging ceremony + v0.1.0 gates. Per `ROADMAP.md` (landed 2026-04-21 on main), v0.1.0 readiness is now gated on [the items in §Pre-v0.1.0 Gates there] rather than on remaining AUDIT work.
