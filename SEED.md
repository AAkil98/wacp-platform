...
**Branch:** `dev`, **26 commits ahead of `main` @ `743c9bd`**. The dev→main batched merge ran 2026-04-17 morning (fast-forward, 21 commits); since then dev has accumulated §13.7.6b WA1/WA2/WA3 + strategy-doc + seed refresh + WA3.5/WA3.6 + WA5 + T7.3 (with WorkspaceState fix) + un-ignore sweep + closure docs + §13.7.9 mutation-testing pipeline + §13.7.7 D1 (Playwright tooling) + inter-deliverable drift-filing + §13.7.7 D2 (five E2E spec files + three backend-drift fixes) + **§2.1–§2.6 CI-pipeline cleanup** + **§2.7 full-fix** (Phases A+B+C+D+E, all 9 drift items + D.1/D.2 debt, ff'd from `ci/cleanup-2.7`) + **§2.7.1b** (14 highway-ui strict-mode errors unmasked by §2.7.4) + **protoc rate-limit hardening** (`repo-token` on 12 `arduino/setup-protoc@v3` usages) + **§13.7.7 D3** (Playwright `e2e` job in `ci-console.yml`) + D4 (e2e/README.md) + D5 (this refresh). **§13.7.6b + §13.7.9 + §2.1–§2.6 CI cleanup + §2.7 in full + §13.7.7 in full all closed.** Working tree clean.

**CI state on `dev`:** all four triggered workflows (`ci-lint`, `ci-wacp`, `ci-console`, `coverage`) are **green**. The `ci-console` workflow now includes the new `e2e` job (Rust + Integration + Frontend + Playwright — 4 parallel jobs, ~3m25s cold-cache for e2e). Remaining CI tasks are future-facing (Playwright `--coverage` merge into lcov, Codecov ratchet); nothing red on `dev`.
...
**Playwright E2E (§13.7.7) — fully landed:** D1 `03d0411` (tooling + `wacp-mock-runtime`), D2 `385ba71` (five spec files: 7 unskipped passing, 9 `test.skip` with UNSKIP WHEN notes), D3 `3508941` (`e2e` job in `ci-console.yml` — green first try, 3m25s cold-cache), D4 (`wacp-console/frontend/e2e/README.md` — local-run + debug + port map), D5 (AUDIT §13.2/§13.5/§13.7.7/§13.8 + this SEED refresh). Remaining: Playwright `--coverage` → frontend lcov merge (deferred, not blocked).

CI pipeline cleanup (§2.1–§2.6 + §2.7 full-fix + §2.7.1b + protoc rate-limit hardening all landed on `dev`) — see `impl/ci-health-2026-04-17.md` + `impl/ci-cleanup-2.7-plan.md`. §2.7 plan executed across five clusters in ~175 min vs the 5.5–7.5 h estimate: A cheap wins ~35 min, B types ~30 min, C python codegen ~25 min, D mechanical strict-mode ~60 min, E debt ~25 min. Full-fix principle held throughout: no `#[allow]` bandaids, no tsconfig exclusions as deferrals, no config relaxations.

**Not yet present (tracked, not regressions):** the five new Rust integration + chaos suites I1–I5 (§13.7.8; I6 landed via §13.7.6), Codecov monthly ratchet (§13.7.10, deferred until the new baseline settles), and Playwright `--coverage` merge. All broken out with deliverables in `AUDIT-2026-04-15.md` §13.7. Supply-chain scanning, the F-series frontend sweep, the Rust branch-coverage sweep (T1–T11), the deterministic LLM stub provider + I6 integration test, §13.7.6b in its entirety, the §13.7.9 mutation-testing pipeline (awaiting first scheduled run), §2.1–§2.6 + §2.7 CI cleanup, and §13.7.7 Playwright E2E in full are all landed.
...
**M0–M7 merger, W1–W7 wiring, runtime implementation audit, §11 pre-release punch list (1–5), §12.1 tooling, §12.2 T1–T11, §12.3 F1–F10, audit §13.7.1–§13.7.5, §13.7.6, §13.7.6b in full, §13.7.9 (mutation-testing pipeline), §13.7.7 D1+D2+D3+D4+D5 (Playwright tooling + 5 E2E spec files + CI stage + README + audit closure), §2.1–§2.6 CI-pipeline cleanup, §2.7 full-fix in full (all 9 drift items + D.1/D.2 debt), §2.7.1b (14 highway-ui strict-mode errors unmasked by §2.7.4), and protoc rate-limit hardening** all complete and green on `dev`. Test totals: `wacp-coordinator` 387, `wacp-workspace` 65, `wacp-runtime` 109, `wacp-types` 45, `console-integration` 12 passing + 0 ignored, `pnpm test:e2e` 7 pass / 9 skip / 0 fail, frontend `pnpm typecheck` 0 errors (was 54), `pnpm test:isolated` 22 files green, `pnpm lint` 0 errors, `wacp-local` 86 tests, `wacp-cli` 132 tests, all 7 ecosystem packages typecheck, `sdk-python` 104 tests. Workspace clippy + fmt clean.

**CI state on `dev`:** all four triggered workflows green. `ci-console` now runs Rust + Integration + Frontend + **E2E (new Playwright stage, 3m25s cold-cache)**. Dev is **26 commits ahead of main** @ `743c9bd`.

**Key docs:**
- `impl/ci-health-2026-04-17.md` — historical record of the §2.1–§2.7 cleanup; frontmatter `status: resolved` once the plan's docs are refreshed to match (small doc-only follow-up).
- `impl/ci-cleanup-2.7-plan.md` — full five-phase plan. Documents the four plan deviations that needed adapting (openapi stdout race, `OperationType` closed-union, `@wacp/local` dist cascade, betterproto path rename).
- `wacp-console/frontend/e2e/README.md` — new; local-run recipe for Playwright.
- `wacp-console/performance-optimization.md` §12.5 — `ProfilesPage` Create-New click unmounts React is still open (30–60 min bisect recommended).
- §11.4 P0 audit pass on remaining Rust-enum-as-i32 sites still open and high-ROI (~30–60 min).

**When resuming:**

**Branch state to start:** `git checkout dev` (tip `<pending>` after this commit, 26 commits ahead of `main`). Working tree clean. All four CI workflows green on dev.

**Primary tracks — in rough ROI order:**
1. **§13.7.8 (Rust integration I1–I5)** — 4–6 h. Fully independent work package: five new Rust integration/chaos suites under `wacp-console/integration/tests/`. I6 already landed via §13.7.6 (`llm_stub_e2e`). Details at `AUDIT-2026-04-15.md` §13.7.8.
2. **§11.4 P0 audit pass** — 30–60 min. High-ROI: sweep remaining Rust-enum-as-i32 sites across the workspace. Good cooldown task between bigger packages.
3. **§12.5 `ProfilesPage` Create-New unmount** — 30–60 min bisect, per `wacp-console/performance-optimization.md` §12.5.
4. **Playwright `--coverage` merge** — 1–2 h: wire Playwright's `--coverage` (or Istanbul instrumentation) → `wacp-console/frontend/coverage/lcov.info` so e2e runs lift the frontend lcov number. Deferred during D3 per the acceptance criterion; pick up when baseline numbers settle.

**Independent / scheduled:**
5. **§13.7.10 (Codecov monthly ratchet)** — still deferred until 2–3 `main` merges land §13.7.6b + §13.7.7 + §2.7 so the baseline settles.
6. **§13.7.9 first-run triage** — next Monday 04:00 UTC cron (surviving mutants → killer tests; equivalent → `// mutants:skip`).

**Merge strategy:** dev is 26 commits ahead of main with every CI check green. Ready to ff `dev` → `main` whenever convenient. Tag `wacp-runtime-v0.1.0` / `wacp-console-v0.1.0` once Rust branch-coverage floor clears 85% and first mutation run hits ≥85% per module (both are post-v0.1 acceptance gates, not pre-merge).
