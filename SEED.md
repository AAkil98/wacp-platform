...
- `impl/closeout-plan.md` — five-phase plan closing the nine open items: P1 pre-launch PR on `ci/pre-launch-closeout` (6 commits landed locally, pushed; ff pending), P2 branch protection, P3 dev→main ff, P4 Bucket B refactor, P5 §12.5 bisect. Draft status until all phases tick.

**When resuming:**

**Branch state to start:** `git checkout ci/pre-launch-closeout` if continuing the closeout P1 work (6 commits `03d195d..7e83da5`, pushed to `aakil98/ci/pre-launch-closeout`, no PR). Otherwise `git checkout dev`. Local `dev` is 1 commit ahead of `aakil98/dev` (`6197263` — closeout-plan scaffold); `aakil98/dev` and `aakil98/main` still at `d0be941` (ff'd 2026-04-18). All four CI workflows were green on dev at that ff; topic-branch push did not trigger CI per git-strategy §7.1.
...
2. ~~**Tech-debt §7 open questions**~~ — **answered 2026-04-19**: delete (Q1, `f75979e`), agree (Q2, `08ff88f`), single PR (Q3, tracked in closeout-plan P4).
3. **Closeout-plan P1 — ff to `dev`** — 6 commits on `ci/pre-launch-closeout` (`03d195d..7e83da5`): sourcemap off, highway-ui deleted, file-size lint+allowlist, .gitmessage+rerere, opt-in pre-push hook, this docs closeout. All local-green; not yet pushed to `aakil98/dev`. Fast-forward via `ff-main`-style ceremony — but target `dev`, not `main` — when ready.
4. **Closeout-plan P2 — branch protection** — GitHub Settings → Branches → rule for `main` + `dev` (linear history, disallow force-push, disallow deletion). Closes git-strategy §11.5 risk class. Can run any time; target before P3 ff so protection is active on the first post-P1 ff.
5. **Closeout-plan P3 — dev → main ff** — per git-strategy §9.3 (`ff-main` skill). Blocked on P1 ffd + P2 configured. Expected to carry the P1 batch + the P2 doc update forward in one ff.
6. **§12.5 `ProfilesPage` Create-New unmount** — 30–60 min bisect, per `HEALTH-LOG.md` §12.5. Orthogonal to P1–P4; closeout-plan P5.
...
8. ~~**Merge `dev` → `main`** (prior)~~ — **done 2026-04-18**. Ff of 48 commits on `d0be941`. Superseded by item 5 above for the next cut.

**Independent / scheduled:**
9. **§13.7.10 (Codecov monthly ratchet)** — still deferred until 2–3 `main` merges land the §13.7.6b + §13.7.7 + §13.7.8 + §2.7 + closeout-plan batch so the baseline settles.
10. **§13.7.9 first-run triage** — next Monday 04:00 UTC cron (surviving mutants → killer tests; equivalent → `// mutants:skip`).
...
12. **Closeout-plan P4 — Bucket B refactor** (post-P3 ff) — ~8–12 h on a dedicated `refactor/file-splits` branch. Split the 9 oversized production files (`init.rs` 2139, `session_monitor.rs` 2120, `session_launcher.rs` 1877, `highway.rs` 1832, `config.rs` 1748, `recovery.rs` 1485, `rest_gateway.rs` 1202, `sessions.rs` 1181, `execution.rs` 1137). Single blame event, behavior-preserving; acceptance: no Rust file >800 lines in scope, `.file-size-allowlist` Rust production section shrunk to ≤1 entry.
