...
**Branches.** `main` at `338e4f2`; `dev` at `ce4a3a2`, 15 commits ahead of main. Today's 2026-04-20 session closed every remaining **closeout-plan** item (P2 + P3 + P4 + P5) in one continuous push:

- **P2** — branch protection configured on `main` + `dev` via `gh api` (linear history + no force-push + no deletion).
- **P3** — ff'd dev → main, range `d0be941..338e4f2` (11 commits: closeout-plan P1 batch + 2 SEED refreshes + P2).
- **P4** — Bucket B refactor landed as 13 commits on `refactor/file-splits`, ff'd to dev (`b076711..55c29ab`): B.1 `init.rs` fully split into 5 files (2139 → 819 lines); B.2/B.3/B.4/B.5 used a test-extraction pattern (inline `#[cfg(test)] mod tests` → sibling file via `#[path]`) because production-only was already cohesive. `.file-size-allowlist` shrunk from 9 production files to 2 warn-only entries (`config.rs` + `routes/highway.rs`). Full per-phase detail in `impl/bucket-b-refactor-plan.md`. 1939 workspace tests pass; fmt + clippy (`--workspace -- -D warnings`) clean.
- **P5** — fixed the §12.5 ProfilesPage `Create New` unmount. Root cause: `/api/roles` returns `PaginatedResponse<T>` = `{items, cursor, has_more}`, but the frontend cast `rolesQuery.data as RoleSummary[]` and called `.map` inside the form's role `<select>`. Crash deferred until the form rendered (Create-New click). Fix unwraps `.items` with a defensive `Array.isArray` branch; golden-path Playwright spec un-skipped and passing.

Full execution log at `impl/closeout-plan.md` §7. The prior (2026-04-18) ff from `d0be941` carried 48 commits spanning §13.7.6b / §13.7.9 / §13.7.7 D1–D5 / §2.1–§2.6 / §2.7 full-fix / §2.7.1b / protoc rate-limit / §13.7.8 I1–I5 / §11.4 P0 + `tech-debt-2026-04-18.md` + `impl/git-strategy.md` — every AUDIT §13.7 package except 13.7.10 closed.

**CI state.** Four workflows (`ci-lint`, `ci-wacp`, `ci-console`, `coverage`) run on each push to `main` + `dev`. Docs-only commits path-filter down to `ci-lint` + `coverage`. `dev` at `7c4ee1d` (P5 fix commit) was fully green on all four workflows; `ce4a3a2` (docs closeout) CI in flight at refresh time — use `gh run list --branch dev --limit 6` to verify. `main` last ran green at `338e4f2`. `ci-console` includes the Playwright `e2e` stage + 50-test integration suite; `ci-lint` `file-size` job passes (6 warnings / 0 fails). Branch protection active on `main` + `dev` (linear-history + no force-push + no deletion; `enforce_admins=false`). One flake observed: `account_lockout_after_five_failed_logins` hit a port-allocation TOCTOU in `RuntimeHarness::pick_port` (two of five ephemeral ports collided); passed cleanly on `gh run rerun --failed`. Filed as HEALTH-LOG §14.1 with holder-listener fix sketch. Remaining CI tasks: Playwright `--coverage` → lcov merge, Codecov ratchet.
...
**closeout-plan P3 — dev → main ff on 2026-04-20** via `ff-main` skill. Range `d0be941..338e4f2`, 11 commits (P1 batch + 2 post-batch SEED refreshes + P2). Tested the new P2 linear-history rule: accepted ff-only push to both branches. Remaining: CI verification on `main` at `338e4f2`.
...
**CI state on `dev` and `main`:** `ci-console` runs Rust + Integration (now 50 tests) + Frontend + E2E (Playwright). **Dev and main are aligned at `338e4f2` (ff'd 2026-04-20)** — closeout-plan P3 closed. Push-trigger CI on `main` at `338e4f2` is in flight at refresh time; last applicable-workflow run on `dev` at the same SHA was green.
...
- `impl/closeout-plan.md` — **all five phases closed 2026-04-20**. P1 `6197263..c07af7f` (2026-04-19); P2 `338e4f2` repo admin + doc; P3 ff dev→main `d0be941..338e4f2`; P4 `b076711..55c29ab` (13 commits on `refactor/file-splits`); P5 `7c4ee1d` (ProfilesPage PaginatedResponse fix). §7 execution log has per-phase SHAs.
- `impl/bucket-b-refactor-plan.md` — sub-plan of closeout P4. Per-file deviation rationale (B.1 keeps `init.rs` name; B.2/B.3 skip the plan's driver/stage split in favor of test-extraction; B.4 does test-extraction + defers production split; B.5 is five mechanical test-extractions). Two follow-ups still open (full production split of `routes/highway.rs` and `config.rs` — both warn-only, not blocking).

**When resuming:**

**Branch state to start:** `git checkout dev`. Check tips with `git rev-parse dev main` + `git log --oneline main..dev | wc -l` (expect ~15 — all closeout-plan P4+P5 work sits on dev until the next ff-main). Check CI with `gh run list --branch dev --limit 6` or `--branch main --limit 6`. Topic branches: `ci/pre-launch-closeout` and `refactor/file-splits` both ff'd to dev + deleted locally; remote copies at `aakil98/ci/pre-launch-closeout` and `aakil98/refactor/file-splits` still live (explicit delete pending user go-ahead).

**Primary tracks — in rough ROI order:**
1. ~~**§11.4 P0 audit pass**~~ — **done 2026-04-18**. Closed-terminal → COMPLETED branch-coverage follow-up still open.
2. ~~**Tech-debt §7 open questions**~~ — **answered 2026-04-19**.
3. ~~**Closeout-plan P1**~~ — **landed 2026-04-19**.
4. ~~**Closeout-plan P2 — branch protection**~~ — **configured 2026-04-20** via `gh api`.
5. ~~**Closeout-plan P3 — dev → main ff**~~ — **done 2026-04-20** at `338e4f2`.
6. ~~**Closeout-plan P4 — Bucket B refactor**~~ — **landed 2026-04-20** (13 commits on `refactor/file-splits`, ff'd to dev). Details in `impl/bucket-b-refactor-plan.md`.
7. ~~**Closeout-plan P5 — §12.5 ProfilesPage**~~ — **fixed 2026-04-20** (commit `7c4ee1d`). Root cause: PaginatedResponse vs bare-array shape mismatch. Golden-path E2E un-skipped.
8. **ff dev → main** — next natural cut point. 15 commits ahead of main; ff via `ff-main` skill once CI green on `ce4a3a2`. Largest chunk carried forward: the Bucket-B refactor (13 commits, visibility widening + 7 test-extractions + 4 handler-splits + plan doc).
9. **Bucket-B follow-up production splits** — ~4–6 h: `routes/highway.rs` (still 1302 lines; split into `gates.rs` + `escalations.rs` + `envelopes.rs`) and `config.rs` (1038 lines; split into `server.rs` + `storage.rs` + `resources.rs` + `mod.rs`). Both warn-only, not blocking. Tracked as allowlist comments + `impl/bucket-b-refactor-plan.md`.
10. **Playwright `--coverage` merge** — 1–2 h: wire Playwright's `--coverage` → `wacp-console/frontend/coverage/lcov.info`. Deferred during D3; pick up when baseline numbers settle.
11. **`pick_port` TOCTOU fix** — ~2–3 h: replace listener-close-then-bind with holder-listener pattern (requires `--listen-fd` CLI mode on `wacp-runtime`) or deterministic port-range partitioning. Known flake documented in HEALTH-LOG §14.1.

**Independent / scheduled:**
12. **§13.7.10 (Codecov monthly ratchet)** — deferred until 2–3 `main` merges land the closeout-plan + Bucket-B batches so the baseline settles. Today's 2026-04-20 ff counts as one; next ff (carrying P4 + P5 + Bucket-B follow-ups) is the second.
13. **§13.7.9 first-run triage** — next Monday 04:00 UTC cron (surviving mutants → killer tests; equivalent → `// mutants:skip`).
14. **Four §13.7.8 deferred sub-scenarios** — each tied to absent infrastructure (mock highway, FaultyDb::drop_reads, REST replay endpoint, schema-evolution fixture). In-file notes in the relevant tests.

**Merge strategy:** dev at `ce4a3a2`, main at `338e4f2` — 15 commits ahead. Next natural cut point is another dev→main ff carrying the Bucket-B refactor + P5 fix + closeout-plan closure. Tag `wacp-runtime-v0.1.0` / `wacp-console-v0.1.0` once Rust branch-coverage floor clears 85% and first mutation run hits ≥85% per module (both post-v0.1 gates, not pre-merge). See `impl/git-strategy.md` §8 for full tagging ceremony + v0.1.0 gates.
